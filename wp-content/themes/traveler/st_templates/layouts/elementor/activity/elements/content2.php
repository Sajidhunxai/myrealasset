...
<div class="filter-fullwidth">
    <div class="toolbar">
        <ul class="toolbar-action-mobile d-block d-sm-none d-md-none">
        ...
        <h2 class="search-string modern-result-string" id="modern-result-string"><?php echo balanceTags(STActivity::inst()->get_result_string()); ?> <div id="btn-clear-filter" class="btn-clear-filter" style="display: none"><?php echo __('Clear filter', 'traveler'); ?></div> </h2>
    </div>
    <?php
    echo st()->load_template('layouts/elementor/activity/elements/top-filter/top-filter');
    ?>
    <div id="modern-search-result" class="modern-search-result">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/elementor/activity/loop/normal-' . esc_attr($style), '', array('top_search' => true));
            }
        }else{
            echo ($style == 'grid') ? '<div class="col-xs-12">' : '';
            echo st()->load_template('layouts/elementor/activity/elements/loop/none');
            echo ($style == 'grid') ? '</div>' : '';
        }
        wp_reset_query();
        ?>
