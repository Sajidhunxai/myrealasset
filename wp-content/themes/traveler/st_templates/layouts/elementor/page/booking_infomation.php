<?php
    ...
    if($page_style == '2'){
        echo stt_elementorv2()->loadView('pages/checkout/booking-information', ['order_code' => $order_code]);
        return;
    }
			...
			<div class="total-section">
				...
				<ul>
                    ...
                    if ($post_type != 'st_flight') {
                        ...
                        <?php
                        if (!empty($extras['value']) and is_array($extras['value']) && count($extras['value']) && ($extra_price != 0)) { ?>
                            <li>
                                <span class="label"><?php echo __('Extra Price', 'traveler'); ?></span>
                                <span class="value"><?php echo TravelHelper::format_money_from_db($extra_price, $currency); ?></span>
                            </li>
