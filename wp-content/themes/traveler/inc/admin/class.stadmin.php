...
if (!class_exists('STAdmin')) {
    ...
    class STAdmin {
        ...
        static function st_add_price($post_id, $type_price = 'default', $price = 0, $start_date = '', $end_date = '', $status = 1, $priority = 0) {
            global $wpdb;
        ...
        function st_update_price($post_id, $type_price = 'default', $price = 0, $start_date = '', $end_date = '', $status = 1, $priority = 0) {
        ...
        function admin_enqueue_scripts($hook) {
            ...
            if (!in_array(get_post_type(), array('product', 'shop_order'))) {
                wp_enqueue_script('select2.js', get_template_directory_uri() . '/js/select2-new/js/select2.full.js', ['jquery'], NULL, TRUE);
                $lang = get_locale();
            ...
            wp_register_script('fullcalendar', get_template_directory_uri() . '/js/fullcalendar-5.8.0/lib/main' . $prefix. '.js', ['jquery', 'moment'], NULL, TRUE);
            wp_register_script('fullcalendar-lang', get_template_directory_uri() . '/js/fullcalendar-5.8.0/lib/locales-all' . $prefix. '.js', ['jquery', 'fullcalendar'], NULL, TRUE);
            wp_register_style('fullcalendar-css', get_template_directory_uri() . '/js/fullcalendar-5.8.0/lib/main' . $prefix. '.css');
        ...
        function init() {
            ...
            STTraveler::load_libs(['admin/class.admin.mail',]);
        ...
        //Create mail
        function run_crate_mail(){

            //Insert [Register] Email to administrator
            if(get_option('check_mail',0) == 1){
                
            } else {
                update_option('check_mail', 1, false);
                $arr_key_mail =[
                    'partner_email_for_admin' => '[Register] Email to administrator',
                    'partner_resend_email_for_admin' => '[Register] Resend email to administrator',
                    'user_register_email_for_admin' => '[Register normal user] Email to administrator',
                    'partner_email_for_customer' => '[Register] Email to partner (when waiting for approved register)',
                    'partner_email_approved' => '[Register] Email to partner (when approved register)',
                    'send_admin_new_request_withdrawal' => '[Request] Email to administrator request withdrawal',
                    'send_admin_approved_withdrawal' => '[Approved] Email to administrator request withdrawal',
                    'send_user_new_request_withdrawal' => '[Request] Email to partner withdrawal',
                    'send_user_approved_withdrawal' => '[Approved] Email to partner withdrawal',
                    'membership_email_admin' => 'Email for admin when have a new membership',
                    'membership_email_partner' => 'Email for partner when have a new membership',
                    'email_for_admin' => 'Email template send to administrator booking',
                    'email_for_partner' => 'Email template send to partner/owner booking',
                    'email_for_partner_expired_date' => 'Email template send to partner when package is expired date',
                    'email_for_customer' => 'Email template for booking info send to customer',
                    'email_for_customer_out_of_depature_date' => 'Email template for notification of departure date send to customer',
                    'email_confirm' => 'Email template for confirm send to customer',
                    'email_approved' => 'Email template to admin about item needs to be approved',
                    'email_has_refund' => 'Email template for cancel booking send to administrator',
                    'email_has_refund_for_partner' => 'Email template for cancel booking send to partner',
                    'email_cancel_booking_success_for_partner' => 'Email template for successful canceled send to partner',
                    'email_cancel_booking_success_for_partner' => 'Email template for successful canceled send to customer',
                ];
                foreach($arr_key_mail as $key=>$value_title){
                    $value_option_by_key = st()->get_option($key, '');
                    if(!empty($value_option_by_key)){
                        $args = array(
                            'post_title' => $value_title,
                            'post_type' => 'st_template_email',
                            'post_content' => $value_option_by_key,
                            'post_status' => 'publish',
                        );
                        $post_id = wp_insert_post($args);
                        if(!is_wp_error($post_id)){
                            st_update_optiontree_by_id_key($key,$post_id);
                        }else{
