...
switch ($current_range){
    case "year":
        ...
        for ($month; $month <= $currentMonth; $month++) {
            ...
            $chartData['labels'][]=date_i18n('M Y',$timestamp);
            if(!empty($sum_query)){
                ST_Order_Item_Model::inst()->select($sum_query);
            }
            if(!empty($whereRaw))
            {
            ...
            ->where($where)
            ->where('created',date('Y-m-d',$timestamp))
                ->get(1)->row();
            $datasets['data'][]= floatval($query['total']);
                ...
                <div class="post-type-item-wrap">
                    <div class="post-type-item" style="background-color:<?php echo esc_attr($data['bg']) ?>">
                        ...
                        <div class="post-type-info">
                            <span class="amount"><?php echo $data['amount']; ?></span>
