...
if ( $enable_tree == 'on' ) {
    $lists     = TravelHelper::getListFullNameLocation( 'st_hotel', -1);
    $locations = TravelHelper::buildTreeHasSort( $lists );
} else {
    $locations = TravelHelper::getListFullNameLocation( 'st_hotel', -1);
}
...
<div class="destination-search border-right">
    <div id="dropdown-destination-hotel" role="menu" class="form-group d-flex align-items-center form-extra-field dropdown field-detination dropdown-toggle <?php if ( $has_icon ) echo 'has-icon' ?>"  data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="true">
        <?php
        if ( $has_icon ) {
        ...
        <div class="st-form-dropdown-icon" >
            ...
            <div class="render">
                ...
                ?>
                <input type="text" onkeyup="stKeyupsmartSearch(this)" autocomplete = "off" id="location_name_hotel" name="location_name" value="<?php echo esc_attr($location_name); ?>" placeholder = "<?php echo esc_attr($placeholder);?>" data-post-type="st_hotel" data-text-no="<?php echo esc_html__('No locations...', 'traveler') ?>"/>
            </div>
    ...
    <div class="dropdown-menu"  aria-labelledby="dropdown-destination-hotel">
