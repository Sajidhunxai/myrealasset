<?php
    ...
    if ( !class_exists( 'ActivityHelper' ) ) {
        class ActivityHelper
        {
            ...
            {
                global $wpdb; 
                $sql = "SELECT * FROM {$wpdb->prefix}st_activity_availability WHERE `post_id` = {$activity_id} AND `status` = 'available' AND `check_in` = $check_in AND  `check_in` = $check_in AND
                    number_booked < (count_starttime * CASE WHEN number > 0 THEN `number` else (number_booked + 1) END)
                 ";
                $result = $wpdb->get_row( $sql, ARRAY_A );
