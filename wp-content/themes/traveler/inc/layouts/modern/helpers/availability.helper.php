...
//
            ...
            if ($order_id) {
                ...
                switch ($post_type) {
                    ...
                    case 'st_activity':
                        ...
                        $sql = $wpdb->prepare("UPDATE {$table} SET number_booked = number_booked - %d WHERE post_id = %d AND (check_in BETWEEN %s AND %s) AND number_booked > 0", $number_had_boooked, $post_id, $order_item['check_in_timestamp'] , $order_item['check_out_timestamp']);
                        $wpdb->query($sql);
                        break;
        ...
        static function _woocommerce_order_status_changed($order_id,$old_status,$new_status)
        {
            ...
            if (!in_array($old_status, ['trash', 'canceled',  'failed', 'refunded', 'wc-cancelled', 'wc-refunded', 'wc-failed'])) {
                if (in_array($new_status, ['trash', 'canceled', 'failed', 'refunded', 'wc-cancelled', 'wc-refunded', 'wc-failed'])) {
