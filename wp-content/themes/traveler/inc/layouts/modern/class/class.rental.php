        ...
        public function st_filter_rental_ajax()
        {
                    ...
                    if(function_exists('check_using_elementor') && check_using_elementor()){
                        if ($version == 'elementorv2') {
                            if(in_array('traveler-layout-essential-for-elementor/traveler-layout-essential-for-elementor.php', apply_filters('active_plugins', get_option('active_plugins'))) && ($agency == 'agency')){
                               echo '<div class="row service-list-wrapper list-style">';
                            }else {
            ...
            if ($query->have_posts()) {
                            ...
                            if ($version == 'elementorv2') {
                                ...
                                if($half_map_show == 'yes'){
                                    if($style === 'grid'){
                                        ...
                                        echo st()->load_template('layouts/elementor/rental/loop/halfmap', $style, ['item_row' => 1]);
                                    }
                ...
                if(in_array('traveler-layout-essential-for-elementor/traveler-layout-essential-for-elementor.php', apply_filters('active_plugins', get_option('active_plugins'))) && ($agency == 'agency')) {
                    ...
                    echo '<div class="panigation-list-new-style pagination moderm-pagination" data-action_service="st_filter_rental_ajax">';
                    echo TravelHelper::paging($query, false); 
                    echo '</div>';
                }
            } else {
                if ($is_popup_map)
...
//                    ";
        ...
        function do_add_to_cart()
        {
            ...
            $data = [
                ...
                'total_bulk_diccount' => !empty($price_sale['total_bulk_diccount']) ? floatval($price_sale['total_bulk_diccount']): 0,
            ];
            if ($form_validate)
        ...
        function _get_where_query($where)
        {
            ...
            if ($price = STInput::get('price_range')) {
                ...
                if ($numberday > 0){
                    $priceobj[0] = $priceobj[0]/$numberday;
                }
                
                $where .= " AND (tb.min_price >= {$priceobj[0]})";
                if (isset($priceobj[1])) {
                    $priceobj[1] = TravelHelper::convert_money_to_default($priceobj[1]);
                    if ($numberday > 0){
                        $priceobj[1] = $priceobj[1]/$numberday;
                    }
                    $where .= " AND (tb.min_price <= {$priceobj[1]})";
                }
            }
                ...
                CASE
					...
					THEN
						CASE
							...
							THEN CAST(tb.min_price AS DECIMAL) - ( CAST(tb.min_price AS DECIMAL) / 100 ) * CAST(tb.discount_rate AS DECIMAL)
							ELSE CAST(tb.min_price AS DECIMAL) - CAST(tb.discount_rate AS DECIMAL)
						END
					WHEN tb.is_sale_schedule = 'on'
        ...
        static function get_price($post_id = false, $information = false)
        {
            ...
            $price = !empty(get_post_meta($post_id, 'min_price', true)) ? get_post_meta($post_id, 'min_price', true) : 0;
