...
class TravelerObject
{
    ...
    function _class_init()
    {
        ...
        add_action( 'admin_init',[$this, 'st_admin_init_hook'] );
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
                ...
                switch ($post_type) {
                    ...
                    case "st_rental":
                        ...
                        $sql = "SELECT
                                ...
                                FROM
                                (
                                    SELECT {$wpdb->prefix}{$post_type}.min_price AS st_rental_price 
                                    FROM
                                    {$wpdb->prefix}{$post_type}
                                    {$join}
                                    {$where}
    ...
    static function check_cancel_able($order_id)
    {
        ...
        $check_order = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}st_order_item_meta where user_id={$user_id} and  order_item_id=" . $order_id . " and `status`!='canceled' and `status`!='wc-cancelled' LIMIT 0,1");
        if (!$check_order) {
            $return = FALSE;
        } else {
