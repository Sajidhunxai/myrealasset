...
if(!class_exists('ST_Woocommerce'))
{
    class ST_Woocommerce
    {
        ...
        function __construct()
        {
            ...
            add_filter( 'woocommerce_paypal_line_item', array($this,'filter_woocommerce_paypal_line_item'), 10,5 );
            add_filter( 'woocommerce_paypal_args', array($this,'custom_override_paypal_email'), 10,2 );
            //Only 1 item in cart
            add_action( 'woocommerce_before_calculate_totals', array($this,'st_keep_only_last_cart_item'), 30, 1 );
        }
        function st_keep_only_last_cart_item( $cart ) {
        ...
        function custom_override_paypal_email( $paypal_args, $order ) {
            if($paypal_args['tax_cart']){
                $paypal_args['tax_cart'] = TravelHelper::convert_money($paypal_args['tax_cart']);
            }
            return $paypal_args;
        }
        function filter_woocommerce_paypal_line_item( $_array_item_name, $item_name, $quantity, $amount, $item_number ) {
            $amount=TravelHelper::convert_money($amount);
            $_array_item_name['amount'] = $amount;
            return $_array_item_name;
        }
